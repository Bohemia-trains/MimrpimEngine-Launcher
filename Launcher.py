...
from funcions_own import update1, save_config, load_config # Importujeme naši funkci pro aktualizaci
...
print("Deficice lokálních proměnných dokončena.")
...
# Zajištění výchozích hodnot, pokud neexistují
...
print("Zajištění výchozích hodnot v config dokončeno.")
print("Kontroluji cestu ke hře...")
# Kontrola a nastavení cesty ke hře
if "path_game" not in config or not os.path.exists(config["path_game"]):
    print("DEBUG: 'path_game' není v konfiguraci nebo cesta neexistuje.")
    path_game = input("Zadejte složku, ve které bude hra: ")
    config["path_game"] = path_game
    save_config(config) # Uložíme nově zadanou cestu
...
if not os.path.exists(config["path_game"]):
    print(f"DEBUG: Složka pro hru '{config['path_game']}' neexistuje, vytvářím ji...")
    os.makedirs(config["path_game"])
    print(f"Adresář '{config['path_game']}' byl vytvořen.")
else:
    print(f"Adresář '{config['path_game']}' již existuje.")
print("Kontrola složky pro hru dokončena.")
print("Definuji URL pro stažení verze hry z GitHubu...")
url_github_game_version = "https://raw.githubusercontent.com/mimrpim/train-game/refs/heads/main/game.version"
...
try:
    ...
    response_github_version.raise_for_status() # Vyvolá HTTPError pro chybové stavy
    ...
    github_game_ver = config["game_version"] # Použijeme lokální verzi jako zálohu
...
if github_game_ver == config["game_version"]:
    ...
    if platform.system() == "Windows":
        # ZMĚNA ZDE: Použití cmd.exe /C pro spolehlivé spuštění run.bat
        # Příkaz je nyní seznam argumentů
    ...
    print(f"Spouštím hru: {start_game_command} v adresáři: {config['path_game']}")
    try:
        print(f"DEBUG: Pokouším se spustit subprocess: {start_game_command} s cwd={config['path_game']}")
        # Předáváme příkaz jako seznam argumentů, shell=False (výchozí)
        subprocess.run(start_game_command, check=True, cwd=config["path_game"])
    ...
    # Aktualizujeme verzi v konfiguraci
    config["game_version"] = github_game_ver
    save_config(config) # Uložíme novou verzi do config.json
    print("DEBUG: Nová verze uložena do config.json.")

    # URL pro stažení aktualizace hry
    url_seznam_souboru = f"https://raw.githubusercontent.com/mimrpim/train-game/refs/heads/Launcher/versions/client/alpha/{github_game_ver}/windows/list.txt"
    ...
    slozka_pro_stahovani_seznamu = "./download"
    ...
    slozka_pro_stahovani_obsahu_seznamu = "download" # Ponecháno jako 'download' pro stahování, update.bat to pak přesune
    ...
    try:
        # Stáhneme soubor list.txt z GitHubu
        ...
        # Uložíme stažený seznam do lokálního souboru
        ...
        # Zajištění existence složky pro stahování seznamu
        ...
        # Teď použijeme stažený lokální seznam pro stahování jednotlivých souborů
        ...
        # --- NOVÁ ČÁST: Smazání souboru list.txt po dokončení ---
    ...
    # ZMĚNA ZDE: Spouštění externího update skriptu a předání cesty ke hře jako argument
    if platform.system() == "Windows":
        # Příkaz je nyní seznam argumentů
        ...
        # Pro Linux/macOS je také dobré cestu uzavřít do uvozovek
    ...
    if platform.system() == "Windows":
        # ZMĚNA ZDE: Spouštění run.bat přímo z adresáře hry pomocí cwd
        start_game_command = ["cmd.exe", "/C", "run.bat"]
        print(f"DEBUG: Systém je Windows, start_game_command po aktualizaci nastaven na: {start_game_command}")
    else: # Předpokládáme Linux nebo macOS
        # ZMĚNA ZDE: Spouštění run.sh přímo z adresáře hry pomocí cwd
        start_game_command = "./run.sh" # Použijeme ./ pro explicitní spuštění z cwd
